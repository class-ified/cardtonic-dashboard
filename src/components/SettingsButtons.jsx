...
const Button = ({Popup, text}) => {
    const [popupOpen, setPopupOpen] = useState(false)

    const handlePopupOpen = (newValue) => {
        setPopupOpen(newValue)
    }

    return (
        <div style={{cursor: "pointer"}} className="settings__body-option" onClick={() => handlePopupOpen(true)}>
            <Popup handlePopupOpen={handlePopupOpen} popupOpen={popupOpen} />
            <h3 className="text-blue text-vbold text-small">{text}</h3>
